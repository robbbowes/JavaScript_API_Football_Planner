...
describe('Database', function() {
  ...
  it('should contain 20 objects', function () {
    assert.strictEqual(20, clubs.length);
  });

  it('should be able to get a club object by club name', function () {
    var foundTeam = clubs.find(function(club) {
      return club.name === "West Ham United FC"
    })
    assert.deepEqual({
            name: "West Ham United FC",
            stadiumPicture: "https://footballtripper.com/wp-content/uploads/2014/07/west-ham-olympic-stadium-cgi-render.jpg",
  ...
  it('should have a latlng for stadium location', function () {
    var foundTeam = clubs.find(function(club) {
      return club.name === "West Ham United FC"
    })
    var westhamLatLng = foundTeam.latLng
    assert.deepEqual([
      51.5387095,
      -0.0166037
    ], westhamLatLng);
